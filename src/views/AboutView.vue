<template>
  <div class="about flex justify-center items-center">
    <div class="px-5 text-center">
      ...
      <br>
      <div class="">
        <VueDatePicker v-model="date"  auto-apply partial-flow :flow="['calendar']"  :enable-time-picker="false"></VueDatePicker>
      </div>
      <div class="mt-3" v-if="date">
        <button @click="validate" class="bg-red-500 hover:bg-white hover:text-red-300 text-white font-bold py-2 px-4 rounded">
          VALIDACIÓN DE AMOR
        </button>
      </div>
      <div class="text-center flex justify-center mt-3">
        <img v-if="status === 1" width="150" src="../assets/images/enojada.webp" alt="">
      </div>
      <div v-if="status === 2"  class="flex justify-between w-100 relative">
      ...
      validate() {
        ...
        if(date === '1997-09-15') {
          ...
          const moveImages = setInterval(function() {
              ...
              if (currentPosition1 >= 35 && currentPosition2 <= 35) {
          ...
          setTimeout(() => {
            this.msge = 'QUIERO QUE SEPAS QUE TE AMO MUCHOOO';
          }, 4500);
          setTimeout(() => {
            this.msge = 'ERES UNA GRAN NOVIA';
          }, 6500);
          setTimeout(() => {
            this.msge = 'LO HICE CON MUCHO AMOR';
